...
FILES=("docker-compose.yml" "Dockerfile" "src/" "web/" "docs/" "selfbot/" "scripts/" "vite.config.ts" "package.json" "tsconfig.json" "tailwind.config.js" "postcss.config.js" "cookies.txt")
...
ssh -p $SSH_PORT $REMOTE_USER@$REMOTE_HOST "mkdir -p $REMOTE_DIR && cd $REMOTE_DIR && sudo -S rm -rf .next bot controller data netflix plex stream types youtube web index.ts config.ts *.md *.lock *.html tsconfig.node.json vite.config.ts tailwind.config.js postcss.config.js scripts docs .github assets bun.lock www.netflix.com.har" && 
