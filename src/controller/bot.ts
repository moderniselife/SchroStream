...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addBooleanOption(option =>
      option.setName('choose_format')
        .setDescription('Choose video format/quality before downloading')
        .setRequired(false)
    ),
  new SlashCommandBuilder()
    .setName('yts')
...
async function handleYouTube(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  const chooseFormat = interaction.options.getBoolean('choose_format') || false;
  await interaction.deferReply();
  ...
  // If format selection is requested, show format picker
  if (chooseFormat) {
    await handleYouTubeFormatSelection(interaction, url, startTimeMs, queueOption);
    return;
  }
...
// Store format selection sessions
const formatSelectionSessions = new Map<string, { 
  url: string; 
  formats: any[]; 
  title: string;
  thumbnail?: string;
...
async function handleYouTubeFormatSelection(
  interaction: ChatInputCommandInteraction, 
  url: string, 
  startTimeMs: number, 
  queueOption: boolean
): Promise<void> {
  const { listVideoFormats, formatFormatDisplay } = await import('../youtube/downloader.js');
  
  await interaction.editReply('🔍 Fetching available formats...');
  
  const result = await listVideoFormats(url);
  if (!result || result.formats.length === 0) {
    await interaction.editReply('❌ Failed to fetch video formats. Try again or use without format selection.');
    return;
  }

  // Filter to video formats only (has video), sorted by quality
  const videoFormats = result.formats.filter(f => f.has_video);
  
  if (videoFormats.length === 0) {
    await interaction.editReply('❌ No video formats available for this URL.');
    return;
  }

  // Store session data
  const sessionId = `${interaction.user.id}-${Date.now()}`;
  formatSelectionSessions.set(interaction.user.id, {
    url,
  ...
  for (const [key, session] of formatSelectionSessions.entries()) {
    if (Date.now() - session.timestamp > SESSION_TIMEOUT) {
      formatSelectionSessions.delete(key);
    }
  }

  await showFormatSelectionPage(interaction, interaction.user.id, 0);
}

async function showFormatSelectionPage(
  interaction: ChatInputCommandInteraction | StringSelectMenuInteraction | ButtonInteraction,
  userId: string,
  page: number
): Promise<void> {
  const session = formatSelectionSessions.get(userId);
  if (!session) {
    const content = '❌ Format selection session expired. Please try again.';
  ...
  const { formatFormatDisplay } = await import('../youtube/downloader.js');
  
  const ITEMS_PER_PAGE = 25;
  const totalPages = Math.ceil(session.formats.length / ITEMS_PER_PAGE);
  const currentPage = Math.max(0, Math.min(page, totalPages - 1));
  session.page = currentPage;

  const startIdx = currentPage * ITEMS_PER_PAGE;
  const endIdx = Math.min(startIdx + ITEMS_PER_PAGE, session.formats.length);
  const pageFormats = session.formats.slice(startIdx, endIdx);

  // Create select menu options
  const options = pageFormats.map((format, idx) => {
    const label = format.resolution.substring(0, 25);
    const description = formatFormatDisplay(format).substring(0, 100);
    return {
      label: `${startIdx + idx + 1}. ${label}`,
  ...
  const selectMenu = new StringSelectMenuBuilder()
    .setCustomId('yt_format_select')
    .setPlaceholder('Select a video format...')
    .addOptions(options);
  ...
  if (totalPages > 1) {
    const navRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder()
        .setCustomId('yt_format_prev')
        .setLabel('◀ Previous')
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(currentPage === 0),
      new ButtonBuilder()
        .setCustomId('yt_format_page')
        .setLabel(`Page ${currentPage + 1}/${totalPages}`)
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(true),
      new ButtonBuilder()
        .setCustomId('yt_format_next')
        .setLabel('Next ▶')
        .setStyle(ButtonStyle.Secondary)
  ...
  const cancelRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder()
      .setCustomId('yt_format_best')
      .setLabel('🚀 Use Best Quality')
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId('yt_format_cancel')
      .setLabel('❌ Cancel')
  ...
  const embed = new EmbedBuilder()
    .setTitle('📺 Select Video Format')
    .setDescription(`**${session.title}**\n\nChoose a format to download. Higher bitrate = better quality.`)
    .addFields(
      { name: 'Total Formats', value: `${session.formats.length}`, inline: true },
      { name: 'Showing', value: `${startIdx + 1}-${endIdx}`, inline: true },
    )
    .setColor(0xFF0000)
    .setThumbnail(session.thumbnail || null);
...
async function handleFormatSelection(interaction: StringSelectMenuInteraction): Promise<void> {
  const userId = interaction.user.id;
  const session = formatSelectionSessions.get(userId);
  
  if (!session) {
    await interaction.reply({ content: '❌ Session expired. Please use /yt again.', ephemeral: true });
    return;
  }

  const formatId = interaction.values[0];
  const selectedFormat = session.formats.find(f => f.format_id === formatId);
  
  if (!selectedFormat) {
    await interaction.reply({ content: '❌ Invalid format selected.', ephemeral: true });
    return;
  }

  await interaction.update({ 
    content: `📥 Downloading with format: **${selectedFormat.resolution}**...`, 
    embeds: [], 
    components: [] 
  });

  // Start download with selected format
  await startFormatDownload(interaction, session, formatId);
}

async function startFormatDownload(
  interaction: StringSelectMenuInteraction | ButtonInteraction,
  session: { url: string; title: string; startTimeMs: number; queueOption: boolean; thumbnail?: string },
  formatId: string | null // null means best quality
): Promise<void> {
  const guildId = interaction.guildId;
  if (!guildId) return;
  ...
  if (!voiceChannel && !session.queueOption) {
    await interaction.editReply('❌ You must be in a voice channel');
    return;
  }

  const { downloadWithFormat, downloadYouTubeVideo } = await import('../youtube/downloader.js');
  ...
  try {
    const downloadFn = formatId 
      ? () => downloadWithFormat(session.url, formatId, {
          onProgress: async (progress: DownloadProgress) => {
            const now = Date.now();
          ...
          onComplete: async () => {
            try {
              await interaction.editReply('✅ Download complete! Starting stream...');
            } catch (e) { /* ignore */ }
          },
          onError: async (error: string) => {
            await interaction.editReply(`❌ Download failed: ${error}`);
      ...
      : () => downloadYouTubeVideo(session.url, {
          onProgress: async (progress: DownloadProgress) => {
            const now = Date.now();
            if (now - lastUpdateTime < UPDATE_COOLDOWN) return;
            lastUpdateTime = now;
            ...
            try {
              await interaction.editReply(
                `📥 **Downloading Video (Best Quality)**\n` +
                `${createProgressBar(progress.percent)}\n` +
                `📊 ${progress.speed} | ⏱️ ETA: ${progress.eta}\n` +
                `📁 Total: ${progress.total}`
              );
    ...
    const downloadedVideo = await downloadFn();
    
    if (!downloadedVideo) {
      return; // Error handled by callback
    }

    // If queue option, add to queue
    if (session.queueOption) {
      const mediaItem: MediaItem = {
        ratingKey: `yt-${Date.now()}`,
        key: session.url,
        type: 'youtube',
      ...
      addToQueue(mediaItem, interaction.user.id);
      await interaction.editReply(`✅ Added to queue: **${downloadedVideo.title}**`);
      return;
    }
    ...
    await interaction.editReply({ content: null, embeds: [embed], components: [controlRow] });
    ...
    console.error('[Controller] Format download error:', error);
    await interaction.editReply(`❌ Failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }

  // Clean up session
  formatSelectionSessions.delete(interaction.user.id);
}
...
async function handleButton(interaction: ButtonInteraction): Promise<void> {
  ...
  switch (interaction.customId) {
    ...
    default: {
      ...
      // Format selection pagination
      else if (interaction.customId === 'yt_format_prev') {
        const session = formatSelectionSessions.get(interaction.user.id);
        if (!session) {
          await interaction.reply({ content: '❌ Session expired', ephemeral: true });
          return;
        }
        await showFormatSelectionPage(interaction, interaction.user.id, session.page - 1);
      }
      else if (interaction.customId === 'yt_format_next') {
        const session = formatSelectionSessions.get(interaction.user.id);
        if (!session) {
          await interaction.reply({ content: '❌ Session expired', ephemeral: true });
          return;
      ...
      else if (interaction.customId === 'yt_format_best') {
        const session = formatSelectionSessions.get(interaction.user.id);
        if (!session) {
          await interaction.reply({ content: '❌ Session expired', ephemeral: true });
          return;
        }
        await interaction.update({ content: '📥 Downloading with best quality...', embeds: [], components: [] });
        await startFormatDownload(interaction, session, null);
      }
      else if (interaction.customId === 'yt_format_cancel') {
        formatSelectionSessions.delete(interaction.user.id);
        await interaction.update({ content: '❌ Format selection cancelled.', embeds: [], components: [] });
      }
    }
  }
}
...
async function handleSelectMenu(interaction: StringSelectMenuInteraction): Promise<void> {
  ...
  if (interaction.customId === 'yt_format_select') {
